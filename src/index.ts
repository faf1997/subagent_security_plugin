...
type ExecGuardConfig = {
  enabled?: boolean;
  // legacy/default (still supported)
  mode?: "denylist" | "allowlist";
  allow?: string[];
  deny?: string[];
  denyWords?: string[];
  ...
  policies?: Record<string, ExecPolicy>;
...
export default function register(api: any) {
  ...
  const compilePolicy = (p: ExecPolicy | undefined): CompiledPolicy => {
    const mode = (p?.mode ?? cfg.mode ?? "denylist") as "denylist" | "allowlist";
    let allow: RegExp[] = [];
    let deny: RegExp[] = [];
    try {
  ...
  const compiledByAgent = new Map<string, CompiledPolicy>();
